...
<Steps>
  ...
  {<h3 />}
  In both the [Card Farming](/docs/settings/card-farming) and [Achievement Unlocker](/docs/settings/achievement-unlocker) settings tabs, you can configure the next task to run automatically by enabling the `schedule next task` option and selecting the task you want to run next.
</Steps>