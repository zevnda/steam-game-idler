    ...
    for (const achievement of achievements) {
      if (isMountedRef.current) {
        ...
        if (
          ...
          (maxAchievementUnlocks && achievementsRemaining <= achievements.length - maxAchievementUnlocks)
        ) {
          await stopIdle(game.appid, game.name)
          await removeGameFromUnlockerList(game.appid)
          logEvent(
...
const startCountdown = (durationInMinutes: number, setCountdownTimer: Dispatch<SetStateAction<string>>): void => {
  try {
