...
export function useMessageOperations({
  ...
  const handleSendMessage = useCallback(
    async (message: string, replyToId?: string | null): Promise<void> => {
      try {
    ...
    [userSummary, setMessages, setShouldScrollToBottom, pagination, supabase, broadcastStopTyping],
  )

  const handleDeleteMessage = useCallback(
    async (msgId: string, msgUserId: string): Promise<string | null | void> => {
      try {
    ...
    [userSummary, userRoles, messages, setMessages, supabase],
  )

  const handleEditMessage = useCallback(
    async (msgId: string, newContent: string): Promise<void> => {
      try {
    ...
    [userSummary, userRoles, messages, setMessages, supabase, handleDeleteMessage],
  )
