...
export const useAutomate = () => {
    ...
    const startCardFarming = () => cardFarming(setActivePage);
    const startAchievementUnlocker = () => achievementUnlocker(setActivePage);

    return { startCardFarming, startAchievementUnlocker };
};

// Start card farming
export const cardFarming = async (setActivePage) => {
    try {
...
export const achievementUnlocker = async (setActivePage) => {
    try {
        // Check if Steam is running
        const steamRunning = await invoke('check_status');
        // Retrieve settings from local storage
        ...
        sendMessage({ achievementUnlocker: true });
    } catch (error) {
