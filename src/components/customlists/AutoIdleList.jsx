...
export default function AutoIdleList() {
    const { list: autoIdleList, setList, visibleGames, filteredGamesList, containerRef, setSearchTerm, handleAddGame, handleRemoveGame } = useCustomList('autoIdleList');
    const { isOpen, onOpen, onOpenChange } = useDisclosure();

    return (
        <Fragment>
            <div className='w-calc min-h-calc max-h-calc overflow-y-auto overflow-x-hidden' ref={containerRef}>
                <div className={`fixed flex justify-between items-center w-[calc(100svw-66px)] py-2 pl-4 bg-base bg-opacity-90 backdrop-blur-md z-10 ${autoIdleList.slice(0, visibleGames).length >= 21 ? 'pr-4' : 'pr-2'}`}>
                    <div className='flex flex-col'>
                        <p className='text-lg font-semibold'>
                            Auto Idle
                        </p>
                        <p className='text-xs text-gray-400'>
                            Add games to this list to automatically idle them
                        </p>
                    </div>
                            ...
                            <ModalBody className='relative p-0 gap-0'>
                                <div className='overflow-y-auto h-[343px]'>
                                    {filteredGamesList && filteredGamesList.map((item) => (
                                        <div
                                            key={item.appid}
                                            className={`flex justify-between items-center gap-2 hover:bg-containerhover cursor-pointer px-3 py-1 duration-150 select-none ${autoIdleList.some(game => game.appid === item.appid) && 'opacity-30'}`}
                                            onClick={() => autoIdleList.some(game => game.appid === item.appid) ? handleRemoveGame(item) : handleAddGame(item)}
                                        >
                                            ...
                                            <div className='flex justify-center items-center'>
                                                {autoIdleList.some(game => game.appid === item.appid) && (
