                            ...
                            <ModalBody className='relative p-0 gap-0'>
                                <div className='overflow-y-auto h-[343px]'>
                                    {filteredGamesList && filteredGamesList.map((item) => (
                                        <div
                                            key={item.appid}
                                            className={`flex justify-between items-center gap-2 hover:bg-containerhover cursor-pointer px-3 py-1 duration-150 select-none ${cardFarmingList.some(game => game.appid === item.appid) && 'opacity-30'}`}
                                            onClick={() => cardFarmingList.some(game => game.appid === item.appid) ? handleRemoveGame(item) : handleAddGame(item)}
                                        >
                                            ...
                                            <div className='flex justify-center items-center'>
                                                {cardFarmingList.some(game => game.appid === item.appid) && (
