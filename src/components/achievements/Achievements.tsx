...
export default function Achievements(): ReactElement {
  ...
  if (isLoading)
    return (
      <div className={cn('overflow-y-auto overflow-x-hidden', 'bg-base border-t border-border w-screen')}>
        <Loader />
      </div>
    )

  return (
    <div className={cn('min-h-calc max-h-calc w-full bg-base', 'overflow-hidden border-t border-border')}>
      <div className='p-4'>
